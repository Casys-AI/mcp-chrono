...
/**
 * Capture docs/assets/chrono-recorded-run-viewer.png from the committed viewer
...
for (
  const required of ["src/ui/dist/run-record-viewer/index.html", harnessPath.slice(1)]
) {
  await Deno.stat(join(root, required)).catch(() => {
    throw new Error(`CAPTURE_INPUT_MISSING ${required} — run deno task build:ui first`);
  });
}
...
const rawScreenshot = join(dirname(outputPath), ".chrono-recorded-run-viewer.raw.png");
try {
