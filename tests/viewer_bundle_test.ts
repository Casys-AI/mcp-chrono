...
Deno.test("built Chrono viewer contains its App-owned whole view", async () => {
  ...
  assert(html.includes("chrono.run-summary"));
  assert(html.includes("chrono.sample-page"));
  assert(html.includes("chrono.execution-facts"));
  assert(html.includes("chrono.receipt-provenance"));
  assert(html.includes("mcp-view-semantic-element"));
  assert(html.includes("casys-chrono-run-record"));
