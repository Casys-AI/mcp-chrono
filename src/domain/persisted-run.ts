...
function observation(value: unknown, input: PrescribedKinematicsCase): RunObservation {
  ...
  if (exit.raw_name !== null && typeof exit.raw_name !== "string") {
    invalid("Persisted exit name is invalid.");
  }
  if (
  ...
  return raw as unknown as RunObservation;
}

export function persistedRecordCaseSha256(value: unknown, requestedId: string): string {
  const raw = object(value);
  exactKeys(raw, ["request", "case_uri", "recorded_at", "output"]);
...
export function validatePersistedRecord(
  ...
  return {
    ...
    output: observation(raw.output, input),
  };
}
