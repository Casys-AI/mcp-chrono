...
export async function auditedViewerDenoConfig(): Promise<unknown> {
  ...
  for (const requirement of REQUIRED_ROOTS) {
    ...
    for (
      const [specifier, relativeEntry] of Object.entries(
        requirement.entries,
      )
    ) {
...
function requiredLocalRoot(environment: string): string {
  ...
  if (!value) {
    throw new Error(
      `Missing ${environment}. Chrono viewer builds require explicit audited local roots for ${
